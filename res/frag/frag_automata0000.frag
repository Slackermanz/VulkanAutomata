...
float sig2(float x, float w) { return 1.0 / ( 1.0 + exp( -w*x + w ) ); }
vec4  sig2(vec4  x, float w) { return 1.0 / ( 1.0 + exp( -w*x + w ) ); }

float sig0(float x) { return 1.0 / ( 1.0 + exp( -x ) ); }
vec4  sig0(vec4  x) { return 1.0 / ( 1.0 + exp( -x ) ); }
...
float getp(int d) 		{ return gl_FragCoord[d] / textureSize(txdata,0)[d]; }
float parv(float w) 	{ return getp(1)*w; }
...
vec4 place( vec4 col, float sz, vec2 mxy, uint s, float off ) {
	...
	float cy = mod(ub.frame+off, 312.0) / 312.0;
	float c2 = mod(ub.frame+off, 717.0) / 717.0;
...
vec4 mouse(vec4 col, float sz) {
	...
	return place(col, sz, mxy, u32_upk(ub.minfo, 4u, 0u), 0.0); }

vec4 symsd(vec4 col, float sz) {
	for(int i = 0; i < 11; i++) {
		col = place(col, (sz/11.0)*((11.0-i)), vec2(textureSize(txdata,0)[0]/2.0,textureSize(txdata,0)[1]/2.0), i&1u, i*ub.frame); }
	return col; }
...
//	----    ----    ----    ----    ----    ----    ----    ----
	...
	if(u32_upk(ub.minfo, 4u, 28u) == 2u)	{ res_c = vec4(0.0,0.0,0.0,0.0); 	}
	if(u32_upk(ub.minfo, 4u, 28u) == 3u)	{ res_c = symsd(res_c, 128.0); 		}
	if(u32_upk(ub.minfo, 4u,  0u) == 1u 
	|| u32_upk(ub.minfo, 4u,  0u) == 3u)	{ res_c = mouse(res_c, 28.0); 		}
