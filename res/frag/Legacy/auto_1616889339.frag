...
//	Parameters
	...
	float b  = mnp * 16.0 *  32.0;
	float n  = mnp * 16.0 *   2.0;
	float cy = mnp * 16.0 * 128.0;
	float li = mnp * 16.0 *  96.0;
	float lu = mnp * 16.0 *  64.0;
...
//	Get Neighbourhood Values
	...
	float[SCNH_COUNT_CHAN] nhv_g;
		...
		nhv_g[4] = nh16_t_02(ivec2(SCUI24, SCUI25), rings_g);
		nhv_g[5] = nh16_t_02(ivec2(SCUI26, SCUI27), rings_g);
		nhv_g[6] = nh16_t_02(ivec2(SCUI28, SCUI29), rings_g);
		nhv_g[7] = nh16_t_02(ivec2(SCUI30, SCUI31), rings_g);
	...
	float[SCNH_COUNT_CHAN] nhv_b;
		...
		nhv_b[4] = nh16_t_02(ivec2(SCUI40, SCUI41), rings_b);
		nhv_b[5] = nh16_t_02(ivec2(SCUI42, SCUI43), rings_b);
		nhv_b[6] = nh16_t_02(ivec2(SCUI44, SCUI46), rings_b);
		nhv_b[7] = nh16_t_02(ivec2(SCUI46, SCUI47), rings_b);
