...
//	Get Neighbourhood Values
/*	vec3 nhr0 = nhd( ivec2(1,   0), origin, psn, 0.0, 0 );
	vec3 nhr1 = nhd( ivec2(2,   0), origin, psn, 0.0, 0 );
	vec3 nhr2 = nhd( ivec2(4,   0), origin, psn, 0.0, 0 );
	vec3 nhr3 = nhd( ivec2(6,  0), origin, psn, 0.0, 0 );
	vec3 nhr4 = nhd( ivec2(8,  0), origin, psn, 0.0, 0 );
	vec3 nhr5 = nhd( ivec2(22,  0), origin, psn, 0.0, 0 );
	vec3 nhr6 = nhd( ivec2(28,  0), origin, psn, 0.0, 0 );
/**/
...
/*	vec3 	n0g 	= nhd( ivec2(4,0), origin, psn, 0.0, 1 );
	vec3 	n0b 	= nhd( ivec2(2,0), origin, psn, 0.0, 2 );
	float 	n0gw 	= n0g[0] / n0g[2];
	float 	n0bw 	= n0b[0] / n0b[2];
	res_g = ( res_g + n0gw * s * 1.0 + res_r * s * 1.2 ) / (1.0 + s * 3.2);
	res_g = res_g * res_r * 0.99;
	res_b = 0.0;//res_g * res_g * res_g * res_r * 1.3;
/**/
