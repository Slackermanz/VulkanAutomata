...
//	Parameters
	float s  = mnp * 16.0 * 128.0;
	float b  = mnp * 16.0 *  96.0;
	float c4 = mnp * 16.0 *  32.0;
	float c8 = mnp * 16.0 *   0.0;
	float n  = mnp * 16.0 *   4.0;
	float cy = mnp * 16.0 * 192.0;
	float li = mnp * 16.0 *  64.0;
	float lu = mnp * 16.0 *   8.0;
...
//	----    ----    ----    ----    ----    ----    ----    ----
	...
	float 	nhr_blur = 0.0;
	for(int i = 0; i < SCNH_COUNT_CHAN; i++) { nhr_blur = nhr_blur + nhv_r[i]; }
			nhr_blur = nhr_blur / float(SCNH_COUNT_CHAN);
	res_r = (res_r + nhr_blur * b) / (1.0 + b);

	float 	nhg_blur = 0.0;
	for(int i = 0; i < SCNH_COUNT_CHAN; i++) { nhg_blur = nhg_blur + nhv_g[i]; }
			nhg_blur = nhg_blur / float(SCNH_COUNT_CHAN);
	res_g = (res_g + nhg_blur * b) / (1.0 + b);

	float 	nhb_blur = 0.0;
	for(int i = 0; i < SCNH_COUNT_CHAN; i++) { nhb_blur = nhb_blur + nhv_b[i]; }
			nhb_blur = nhb_blur / float(SCNH_COUNT_CHAN);
...
//	Interpolate
	float	inp_r = (res_r 	  	* 1.0 	+ nhg_blur 	*  li 	+ nhb_blur 	*  li	) / ( 1.0 + li * 2.0 );
	float	inp_g = (nhr_blur 	*  li 	+ res_g 	* 1.0 	+ nhb_blur 	*  li	) / ( 1.0 + li * 2.0 );
	float	inp_b = (nhr_blur 	*  li	+ nhg_blur 	*  li 	+ res_b 	* 1.0	) / ( 1.0 + li * 2.0 );
	res_r = inp_r;
	res_g = inp_g;
	res_b = inp_b;
/**/
...
//	Random
	float[6] rnw;
		rnw[0] = eval4_ubi_f[0]  * c4;
		rnw[1] = eval4_ubi_f[1]  * c4;
		rnw[2] = eval4_ubi_f[2]  * c4;
		rnw[3] = eval4_ubi_f[3]  * c4;
		rnw[4] = eval4_ubi_f[4]  * c4;
		rnw[5] = eval4_ubi_f[5]  * c4;
	float 	rnc_r = (res_r * 1.0 	+ res_g * rnw[0] 	+ res_b * rnw[1] ) / (1.0 + (rnw[0]+rnw[1]));
	float 	rnc_g = (res_r * rnw[3]	+ res_g * 1.0 		+ res_b * rnw[2] ) / (1.0 + (rnw[2]+rnw[3]));
	float 	rnc_b = (res_r * rnw[4]	+ res_g * rnw[5] 	+ res_b * 1.0	 ) / (1.0 + (rnw[4]+rnw[5]));
	res_r = rnc_r; res_g = rnc_g; res_b = rnc_b;

		rnw[0] = eval4_ubi_f[6]  * c4;
		rnw[1] = eval4_ubi_f[7]  * c4;
		rnw[2] = eval4_ubi_f[8]  * c4;
		rnw[3] = eval4_ubi_f[9]  * c4;
		rnw[4] = eval4_ubi_f[10] * c4;
		rnw[5] = eval4_ubi_f[11] * c4;
	 		rnc_r = (res_r * 1.0 	+ res_g * rnw[0] 	+ res_b * rnw[1] ) / (1.0 + (rnw[0]+rnw[1]));
	 		rnc_g = (res_r * rnw[3]	+ res_g * 1.0 		+ res_b * rnw[2] ) / (1.0 + (rnw[2]+rnw[3]));
		 	rnc_b = (res_r * rnw[4]	+ res_g * rnw[5] 	+ res_b * 1.0	 ) / (1.0 + (rnw[4]+rnw[5]));
	res_r = rnc_r; res_g = rnc_g; res_b = rnc_b;
/**/
//	Cyclic
	float[6] cyw;
