...
//	rslt[6] = n6r[0] / n6r[2];
	...
	float 	cdx 			= 	 	( fc[0] - dm[0] * 0.5 ) / (dm[0] * 0.5);
	float 	cdy 			= 	 	( fc[1] - dm[1] * 0.5 ) / (dm[1] * 0.5);
...
//		BLUR Section altered for Neighbourhood Offset Test
	...
	res_r = (res_r + blrt * s * blr_sz * 0.2) / (1.0 + s * blr_sz * 0.2);
...
//	----    ----    ----    ----    ----    ----    ----    ----
	...
	res_g = ( res_g + n0gw * s * 1.0 + res_r * s * 1.0 ) / (1.0 + s * 2.0);
	res_b = ( res_b + n0bw * s * 0.5 + res_r * s * 0.5 ) / (1.0 + s * 1.0);
